...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	case WM_KEYDOWN:
		...
		if (wParam == VK_UP) Up = true;
		if (wParam == VK_DOWN) Down = true;
		break;
	case WM_KEYUP:
		if (wParam == VK_LEFT) Left = false;
		if (wParam == VK_RIGHT) Right = false;
		if (wParam == VK_UP) Up = false;
		if (wParam == VK_DOWN) Down = false;
		break;
	case WM_DESTROY:
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
	...
	while (WM_QUIT != msg.message) {
		...
		if (PeekMessage(&msg, nullptr, 0, 0, PM_REMOVE)) {
			...
			if (Left) game.posX -= 0.00005f;
			if (Right) game.posX += 0.00005f;
			if (Up) game.posY += 0.00005f;
			if (Down) game.posY -= 0.00005f; //키 눌림에따른 좌표 계산

			Vertex updated[6];//새로운 vertices생성
			for (int i = 0; i < 6; i++) {
				updated[i] = vertices[i];
				updated[i].x += game.posX;
				updated[i].y += game.posY;
			}//좌표 반영
			...
			D3D11_BUFFER_DESC bd = { sizeof(updated), D3D11_USAGE_DEFAULT, D3D11_BIND_VERTEX_BUFFER, 0, 0, 0 };
			D3D11_SUBRESOURCE_DATA initData = { updated, 0, 0 };
			g_pd3dDevice->CreateBuffer(&bd, &initData, &pVBuffer);//updated로 새로운버퍼생성
